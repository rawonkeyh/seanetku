...
<body>
    <div class="container">
        ...
        <div id="pending" class="card" style="display:none;">
            ...
            <button id="resume-btn" class="btn btn-primary">💳 Lanjutkan Pembayaran</button>
            <button onclick="location.reload()" class="btn btn-secondary" style="margin-bottom:10px;">🔄 Cek Status</button>
    ...
    <script>
        ...
        function resolveToken() {
            ...
            if (active && active.order_id === orderId) {
        ...
        async function checkTransactionStatus() {
            try {
                ...
                const headers = token ? {'X-Transaction-Token': token} : {};
        ...
        function showPending(transaction) {
            ...
            resumeBtn.style.display = (active && active.order_id === transaction.order_id) ? 'block' : 'none';
        }
        ...
        function setupResumeButton() {
            ...
            button.addEventListener('click', () => {
                const active = getActivePayment();
                if (!active || active.order_id !== orderId) {
                    alert('Sesi pembayaran tidak ditemukan atau sudah habis. Silakan ajukan ulang dari beranda.');
                    return;
                }

                snap.pay(active.snap_token, {
                    onSuccess: function() {
                        localStorage.removeItem(ACTIVE_PAYMENT_KEY);
                        location.reload();
                    },
                    ...
                    onError: function() {
                        alert('Pembayaran belum berhasil. Anda bisa mencoba lagi sebelum waktu habis.');
                    },
                    onClose: function() {
                        alert('Jendela pembayaran ditutup. Anda tetap bisa melanjutkan selama belum melewati batas waktu.');
                    }
                });
            });
        }
