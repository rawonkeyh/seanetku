...
class TransactionService
{
    ...
    public function createTransaction(array $data): array
    {
        return DB::transaction(function () use ($data) {
            ...
            return [
                ...
                'payment' => $paymentData,
                'access_token' => $transaction->accessToken(),
                'payment_expires_at' => now()
                    ->addMinutes((int) config('voucher.reservation_timeout', 15))
                    ->toIso8601String(),
            ];
        });
    }
    ...
    public function handlePaymentFailure(string $orderId): bool
    {
        return DB::transaction(function () use ($orderId) {
            ...
            // Never downgrade a paid transaction due to delayed/out-of-order events.
            if ($transaction->isPaid()) {
                Log::warning('Skip failure transition because transaction is already paid', [
                    'order_id' => $orderId,
                    'transaction_id' => $transaction->id,
                ]);
                return true;
    ...
    public function handlePaymentExpired(string $orderId): bool
    {
        return DB::transaction(function () use ($orderId) {
            ...
            // Never downgrade a paid transaction due to delayed/out-of-order events.
            if ($transaction->isPaid()) {
                Log::warning('Skip expired transition because transaction is already paid', [
                    'order_id' => $orderId,
                    'transaction_id' => $transaction->id,
                ]);
                return true;
